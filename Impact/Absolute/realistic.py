...
def visualise_probabilities(topologies, info, results):
    ...
    rects1 = ax.bar(index + (bar_width * 0),
                    ...
                    yerr=list(results[1].values()))

    rects2 = ax.bar(index + (bar_width * 1),
                    list(results[2].values()),
                    bar_width,
                    alpha=opacity,
                    color='blue',
                    label='Random VNF allocation',
                    yerr=list(results[3].values()))

    rects3 = ax.bar(index + (bar_width * 2),
                    list(results[4].values()),
                    bar_width,
                    alpha=opacity,
                    color='green',
                    label='Topology-aware VNF allocation',
                    yerr=list(results[5].values()))

    # Set the title data.
    plt.title('Path lengths from source to target over {} VNF(s)'.
